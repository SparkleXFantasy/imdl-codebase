...
class OtterForConditionalGeneration(OtterPreTrainedModel):
    ...
    def __init__(
        ...
        if "llama" not in config.text_config._name_or_path:
            if config.text_config.architectures[0] == "MPTForCausalLM":
                text_tokenizer = AutoTokenizer.from_pretrained("/home/aya/workspace/hub/weights/gpt/mpt-7b-instruct")
                lang_encoder = MPTForCausalLM(config=config.text_config)
    ...
    def encode_vision(self, vision_input: torch.Tensor):
        B, C, H, W = vision_input.shape
        with torch.no_grad():
            vision = self.vision_encoder(vision_input)[0][:, 1:, :]
        vision = rearrange(vision, "(b T F) v d -> b T F v d", b=B, T=1, F=1)
        vision = self.perceiver(vision)
        return vision
    
    def set_condition_vision(self, vision: torch.Tensor):
        for layer in self.lang_encoder._get_decoder_layers():
