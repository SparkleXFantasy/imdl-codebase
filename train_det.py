...
def img_trans(config):
    ...
    if data_config.crop_enabled:
        trans.append(transforms.Resize((data_config.resize, data_config.resize)))
...
if __name__ == '__main__':
    ...
    imdl_det_dataset = FolderDataset(config.train.dataset.data_root, transform=data_trans)
    train_dataset, val_dataset, test_dataset = random_split_dataset(imdl_det_dataset, config.train.dataset.split)
    ...
    for epoch in range(config.train.epoch):
        ...
        with tqdm(total=len(train_dataset)) as train_pbar:
            ...
            for id, data in enumerate(train_dataloader):
                ...
                det_logits, f4, f3, f2, f1, f0, map = model(img_t, det_only=True)
                ...
                loss_det = BCELoss(det_logits, cls)
